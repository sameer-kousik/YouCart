...
# BASE_DIR = os.path.dirname(os.path.abspath(__file__)) # Commented out
# LOCATION_MAP_FILE = os.path.join(BASE_DIR, "kroger_location_map.json") # Commented out

router = APIRouter()

# def load_location_map() -> Dict[str, str]: # Commented out
...
# def save_location_map(data: Dict[str, str]) -> None: # Commented out
#     try:
...
# token_to_location_id_map: Dict[str, str] = load_location_map() # Commented out initialization
token_to_location_id_map: Dict[str, str] = {} # Initialize as empty dict, no longer loaded from file.
...
# @router.post("/save-location") # Commented out endpoint decorator
# async def save_location_route(location_data: LocationSaveRequest, token: str = Depends(get_current_user_token)): # Commented out function signature
#     # user_id = request.client.host # Removed
#     # if user_id not in user_tokens: # Removed
#     #     raise HTTPException(status_code=401, detail="User not logged in")

#     # token_to_location_id_map[token] = location_data.location_id # Commented out logic
#     # save_location_map(token_to_location_id_map) # Commented out logic
#     # return {"message": "Location saved successfully", "token_used_as_key": token, "location_id_saved": location_data.location_id }
#     raise HTTPException(status_code=404, detail="This endpoint is deprecated. Location is handled client-side or per request.")
